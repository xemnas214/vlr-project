...
parser.add_argument(
    ...
    type="checked_dir",
    metavar="DIR",
    help="extra data directory for validation",
)
parser.add_argument(
    "--extra-data-scenes-json",
    type="checked_file",
    nargs="+",
    default=None,
    metavar="FILE",
...
def main_train(train_dataset, validation_dataset, extra_dataset=None):
    ...
    if extra_dataset is not None:
        extra_dataloader = extra_dataset.make_dataloader(
            args.batch_size,
            shuffle=False,
            drop_last=False,
    ...
    if args.evaluate:
        ...
        if extra_dataset is not None:
            validate_epoch(
                0, trainer, extra_dataloader, meters, meter_prefix="validation_extra"
            )
        logger.critical(
        ...
        logger.critical(
            meters.format_simple(
                "Epoch = {}".format(epoch),
                {
                    ...
                    if epoch % args.validation_interval == 0
                    or not k.startswith("validation")
                },
                compressed=False,
            )
        )
