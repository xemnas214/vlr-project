...
__all__ = ["NSCLDataset", "ConceptRetrievalDataset",
...
class NSCLDatasetUnwrapped(FilterableDatasetUnwrapped):
    def __init__(
        ...
        if isinstance(self.questions_json, (tuple, list)):
            ...
            logger.info('Loading questions from: "{}".'.format(
                self.questions_json))
    ...
    def _get_metainfo(self, index):
        ...
        question["video_folder"] = gdef.get_video_folder(scene)
        question["question_index"] = index
        question["question_tokenized"] = nltk.word_tokenize(
            question["question"])
        ...
        if has_program:
            question["program_tree"] = nsclseq_to_nscltree(
                question["program_seq"])
            question["program_qsseq"] = nsclseq_to_nsclqsseq(
                question["program_seq"])
    ...
    def __getitem__(self, index):
        ...
        if self.image_root is not None and feed_dict.video_folder is not None:
            ...
            for name in glob.glob(
                osp.join(self.image_root, feed_dict.video_folder) + "/*.png"
            ):
                image = Image.open(name).convert("RGB")
                image, _ = self.image_transform(
                    image, original_objects
                )
                video += [image]
        ...
        feed_dict.answer = gdef.canonize_answer(
            metainfo.answer, metainfo.question_type)
...
class NSCLDatsetFilterableView(FilterableDatasetView):
    ...
    def filter_question_type(self, *, allowed=None, disallowed=None):
        ...
        if allowed is not None:
            return self.filter(
                filt,
                "filter-question-type[allowed={{{{}}}]".format(
                    ",".join(list(allowed))),
            )
        elif disallowed is not None:
            return self.filter(
            ...
            raise ValueError(
                "Must provide either allowed={...} or disallowed={...}.")
...
class ConceptRetrievalDatasetUnwrapped(FilterableDatasetUnwrapped):
    ...
    def _get_metainfo(self, index):
        ...
        question["video_folder"] = gdef.get_video_folder(question["scene"])
