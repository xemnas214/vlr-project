...
class ReasoningV1Model(nn.Module):
    def __init__(self, vocab, configs):
        ...
        self.lstm_video = nn.LSTM(input_size=24576, hidden_size=256, num_layers=1, bidirectional=False, batch_first=True)
        ...
        self.reasoning = qs.DifferentiableReasoning(
            self._make_vse_concepts(
                configs.model.vse_large_scale, configs.model.vse_known_belong
            ),
        ...
        self.scene_loss = vqa_losses.SceneParsingLoss(
            gdef.all_concepts, add_supervision=configs.train.scene_add_supervision
        )
    ...
    def _make_vse_concepts(self, large_scale, known_belong):
        if large_scale:
            return {
                "attribute_ls": {
                    "attributes": list(gdef.ls_attributes),
                    "concepts": list(gdef.ls_concepts),
                },
                "relation_ls": {
                    "attributes": None,
                    "concepts": list(gdef.ls_relational_concepts),
                },
        ...
        return {
            "attribute": {
                ...
                "concepts": [
                    ...
                    for k, vs in gdef.attribute_concepts.items()
                    for v in vs
                ],
            },
            "relation": {
                ...
                "concepts": [
                    (v, k if known_belong else None)
                    for k, vs in gdef.relational_concepts.items()
                    for v in vs
                ],
            },
        }
