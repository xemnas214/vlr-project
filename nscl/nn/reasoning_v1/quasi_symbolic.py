...
__all__ = [
    "ConceptQuantizationContext",
    "ProgramExecutorContext",
...
def set_apply_self_mask(key, value):
    logger.warning(
        "Set {}.apply_self_mask[{}] to {}.".format(
            set_apply_self_mask.__module__, key, value
        )
    )
...
class ConceptQuantizationContext(nn.Module):
    def __init__(
        self, attribute_taxnomy, relation_taxnomy, training=False, quasi=False
    ):
        """
        Args:
        ...
        for i in range(batch_size):
            ...
            for concept in self.attribute_taxnomy.all_concepts:
                ...
                if self.quasi:
                    ...
                    output_list[i]["filter"][concept] = (
                        (scores > 0).nonzero().squeeze(-1).cpu().tolist()
                    )
            ...
            for attr in self.attribute_taxnomy.all_attributes:
                ...
                if self.quasi:
                    output_list[i]["relate_ae"][attr] = (
                        cross_scores.detach().cpu().numpy()
                    )
                else:
                    cross_scores = cross_scores > 0
                    output_list[i]["relate_ae"][attr] = (
                        cross_scores.nonzero().cpu().tolist()
                    )
            ...
            for attr in self.attribute_taxnomy.all_attributes:
                ...
                if self.quasi:
                    ...
                    output_list[i]["query"][attr] = [
                        idx2word[v] for v in argmax.cpu().tolist()
                    ]
            ...
            for concept in self.relation_taxnomy.all_concepts:
                ...
                if self.quasi:
                    ...
                    output_list[i]["relate"][concept] = (
                        (scores > 0).nonzero().cpu().tolist()
                    )
...
class ProgramExecutorContext(nn.Module):
    def __init__(
        ...
        self.parameter_resolution = ParameterResolutionMode.from_string(
            parameter_resolution
        )
    ...
    def unique(self, selected):
        if (
            self.training
            or _test_quantize.value < InferenceQuantizationMethod.STANDARD.value
        ):
            return jacf.general_softmax(
                selected, impl="standard", training=self.training
            )
        # trigger the greedy_max
        return jacf.general_softmax(
            selected, impl="gumbel_hard", training=self.training
        )
    ...
    def count_equal(self, selected1, selected2):
        if (
            ...
            b = torch.sigmoid(selected2).sum(dim=-1)
            return (
                (2 * self._count_margin - (a - b).abs())
                / (2 * self._count_margin)
                / self._count_tau
            )
        else:
            return -10 + 20 * (self.count(selected1) == self.count(selected2)).float()

    def query_cp(self, events, obj):
        if obj is not None:
            return obj[0]
        else:
            return None
        #     mask = self._get_concept_groups_masks(concept_groups, 1)
        # mask = torch.min(obj.unsqueeze(0), mask)
        # if torch.is_tensor(obj):
        #     return (mask * obj.unsqueeze(1)).sum(dim=0)
        # return mask[obj]
    ...
    def query_ls_mc(self, selected, group, attribute_groups, concepts):
        mask, word2idx = self._get_attribute_query_ls_mc_masks(
            attribute_groups, concepts
        )
        mask = (mask * selected.unsqueeze(-1).unsqueeze(0)).sum(dim=-2)
        if torch.is_tensor(group):
            return (mask * group.unsqueeze(1)).sum(dim=0), word2idx
    ...
    def _get_attribute_query_masks(self, attribute_groups):
        if self._attribute_query_masks is None:
            ...
            for attribute in attribute_groups:
                mask, this_word2idx = self.taxnomy[1].query_attribute(
                    self.features[1], attribute
                )
                masks.append(mask)
                # sanity check.
    ...
    def _get_attribute_query_ls_mc_masks(self, attribute_groups, concepts):
        if self._attribute_query_ls_mc_masks is None:
            masks, word2idx = list(), None
            for attribute in attribute_groups:
                mask, this_word2idx = self.taxnomy[1].query_attribute_mc(
                    self.features[1], attribute, concepts
                )
                masks.append(mask)
                word2idx = this_word2idx
...
class DifferentiableReasoning(nn.Module):
    def __init__(
        ...
        for i, nr_vars in enumerate(["attribute_ls", "relation_ls"]):
            ...
            setattr(
                self,
                "embedding_" + nr_vars.replace("_ls", ""),
                concept_embedding_ls.ConceptEmbeddingLS(
                    self.input_dims[1 + i],
    ...
    def forward(self, batch_features, progs, fd=None):
        ...
        for i, (features, prog) in enumerate(zip(batch_features, progs)):
            ...
            for block_id, block in enumerate(prog):
                ...
                for inp, inp_type in zip(
                    block["inputs"], gdef.operation_signatures_dict[op][1]
                ):
                    inp = buffer[inp]
                    if inp_type == "object":
                        inp = ctx.unique(inp)
                ...
                if op == "filter":
                    buffer.append(
                        ctx.filter(
                            *inputs, block["concept_idx"], block["concept_values"]
                        )
                    )
                    ...
                    buffer.append(ctx.filter(*inputs, None, None))

                elif op == "filter_frame":
                    inputs = [
                        10
                        + torch.zeros(
                            features[1].size(0),
                    ...
                    buffer.append(
                        ctx.filter_most(
                            *inputs,
                            block["relational_concept_idx"],
                            block["relational_concept_values"],
                        )
                    )
                elif op == "relate":
                    buffer.append(
                    ...
                    buffer.append(
                        ctx.relate_ae(
                            *inputs, block["attribute_idx"], block["attribute_values"]
                        )
                    )
                elif op == "query_collision_partner":
                    # print(inputs)
                    buffer.append(
                        ctx.query_cp(
                            *inputs,
                    ...
                    assert (
                        block_id == len(prog) - 1
                    ), "Unexpected query operation: {}. Are you using the CLEVR-convension?".format(
                        op
                    )
                    if op == "query":
                        # print(inputs)
                        ...
                        raise NotImplementedError(
                            "Unsupported operation: {}.".format(op)
                        )

                if (
                    not self.training
                    and _test_quantize.value
